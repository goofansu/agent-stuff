...
* [`librarian.ts`](pi/extensions/librarian.ts) - Cross-repository code research subagent. Spawns an isolated `pi` process with anthropic/claude-sonnet-4-5 to search and read code across GitHub repositories using `gh` CLI. Invokable as a tool by the agent or directly via `/librarian`.
